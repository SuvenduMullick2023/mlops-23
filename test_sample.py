...
def test_hparam_combination_count():
    ...
    assert not len(h_param_combination)== len(gama_list) * len(C_list)
...
def test_hparam_combination_check():
    ...
    h_param_comb1 = {'gamma':0.001,'cList': 20.0}
    h_param_comb2 = {'gamma':0.01,'cList': 1.0}
    assert (h_param_comb1 in h_param_combination and h_param_combination )